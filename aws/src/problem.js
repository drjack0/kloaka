...
module.exports.trigger = async function (event, context, callback) {
    ...
    try{
        ...
        if(post_sensor.Item){
            const result = post_comparator(post_sensor, data);
            if(result !== "OK"){
                var problem = {
...
function post_comparator(post_sensor, sensor){
    ...
    if(post_sensor_lastMes.filling > sensor_lastMes.filling){
        return "PROBLEM"
    } else if (post_sensor_lastMes.filling < sensor_lastMes.filling){
        return "OBSTRUCTION"
    } else {
        return "OK"
    }
